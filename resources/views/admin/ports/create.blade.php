            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-150 p-8">
                <form method="POST" action="{{ route('ports.store') }}">
                    ...
                    <div class="mt-4">
                        ...
                        <input id="latitude" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="number" step="0.00000001" name="latitude" :value="old('latitude')" required />
                        @error('latitude')
                            <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <input id="longitude" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="number" step="0.00000001" name="longitude" :value="old('longitude')" required />
                        @error('longitude')
                            <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <input id="type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="text" name="type" placeholder="Contoh: Seaport, Container Terminal" :value="old('type')" />
                        @error('type')
                            <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
